...
/**
 * docs/09 Piece 3 — the rich, multi-firm demo dataset. Replaces (does not
 * delete) tools/seed_demo_data.php's single-firm demo with a platform-scale
 * dataset: 1 SaaS-provider super_admin, 4 firms across the firm-level role
 * hierarchy (docs/09 Piece 2), 16 advisor-tier employees, and 160 clients
 * spanning realistic Indian age brackets and life stages — enough variety
 * for the advisor dashboard's health-signal filters (Needs attention /
 * sort-by-readiness) to have something real to demonstrate.
 *
 ...
 *
 * CLI entry point at the bottom of this file guards the direct
 * `php tools/seed_demo_data_full.php` invocation; api/demo_reset.php instead
 * requires this file (which only defines functions/constants when not
 * invoked as the CLI script — see the SEED_DEMO_DATA_FULL_NO_AUTORUN guard
 * below) and calls seedDemoDataFull($db) directly, in-process, so a reset
 * doesn't need to shell out to a second PHP process.
 *
...
// api/demo_reset.php requires() this file with this constant already defined
// (before requiring), so only the CLI entry point at the very bottom is
// skipped — every function/constant above it is still defined either way.
