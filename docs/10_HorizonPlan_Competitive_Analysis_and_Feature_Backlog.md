...
- **P0-5 · Unified client cash-flow view.** *(Built.)* A genuine gap: HorizonPlan modelled saving only per-goal (`base_plans.monthly_sip_amount`/`sip_step_up_rate`, accumulation) and spending only implicitly (`withdrawal_rate`, decumulation), and the portfolio ledger (`client_portfolio_items`) is a *balance sheet* (asset|liability, net worth) — there was no income or monthly-expense field anywhere, so an advisor couldn't see "salary + other income − living expenses − EMIs = monthly surplus" in one place, nor tie that surplus to the SIPs the plan assumes. Shipped as a **cash-flow module**: a new `cash_flow_items` table (sql/030, `kind` income|expense, `label`, `amount`, `cadence` monthly|annual, `category`, `is_active`), tenant-scoped via `TenantScopedDb` and per-client on every row. A pure helper (`api/lib/CashFlowSummary.php`) normalises every line to a monthly figure (annual ÷ 12), sums income/expense/surplus, and — for the advisor — compares the surplus against the client's total goal SIPs ("*is the plan actually fundable from cash flow?*", surplus ₹X vs SIPs ₹Y, gap ₹Z). Endpoints `cash_flow_list.php` (advisor: any client in tenant + SIP comparison; client: own rows only, **no** SIP framing leaked), `cash_flow_upsert.php`/`cash_flow_delete.php` (advisor-only writes), plus a **household roll-up** (`household_cash_flow.php`, the sum of members' own statements — same "grouping, not a shared pool" principle as households/portfolio). Advisor card on the client page (add/edit/delete + live surplus + SIP gap), client-facing read-only card under the DisclosureBanner, and a combined-cash-flow section on the household page. Firm/advisor supplies every number — the app never fabricates a figure; default state is empty, no backfill. *Closes a real planning gap; deepens the moat (the advisory "can they afford it?" question incumbents leave to spreadsheets).* Medium.
...
- **P1-4 · Insurance & liability adequacy module** — *(Built, as the "financial foundations" check.)* Term/health cover adequacy and loan cost in the plan, kept firmly *planning-side, not distribution* — no product, insurer or premium appears anywhere. Four checks in `api/lib/FinancialFoundations.php` (pure, no DB): emergency reserve in months of expenses, income-replacement cover as a multiple of income, medical cover against a reference floor, and any liability whose rate exceeds the plan's own return assumption. The last needs no reference point at all — it compares two figures the user already recorded, which is arithmetic rather than opinion. Only `client_protection` (migration 034: `dependants_count`, `term_life_cover`, `health_cover`, all nullable) is new storage; reserve and debt are read from `cash_flow_items` and `client_portfolio_items` (which gains a nullable `interest_rate`) rather than re-recorded, so figures cannot drift between copies. **Surfaced to both audiences** — advisor card on the client page, plain-language card for a self-serve individual — because the individual tier has nobody to supply this judgement, which is the gap that motivated building it now. Thresholds are sourced reference points in the `strategyPresets.js` framing (3–6 months; 10–15× income; ₹5L floor), never recommendations. The design turns on three states staying distinct: `not_recorded` never renders as a pass, `not_applicable` (no dependants) is not a failure, and an untouched ledger is not a debt-free pass. Also captions the readiness score with what it does not cover. *Deepens the moat.* Medium.
